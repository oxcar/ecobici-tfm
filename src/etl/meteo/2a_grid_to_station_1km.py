"""
Interpolación de datos meteorológicos de cuadrícula a estaciones usando IDW.
"""
import os
import sys
...
GRID_MAPPING_PATH = DATA_DIR / "meteo/grid/grid_1km.csv"
STATIONS_PATH = DATA_DIR / "ecobici/stations/0_stations.csv"
GRID_PARQUET_DIR = DATA_DIR / "meteo/grid/1km"
STATION_PARQUET_DIR = DATA_DIR / "meteo/station/1km"
...
def main():
    ...
    # La estructura es grid_id=X/weather.parquet, así que particionamiento Hive podría funcionar automáticamente
    ...
    # Sin columnas categoricas en esta version (sin weather_code)
    categorical_cols = []
    ...
    # Recolectar todo el dataset podría ser pesado (500 estaciones * meses * horario).
    ...
    for i, station_code in enumerate(unique_stations):
        ...
        # Agregar columna station_code y aplicar zona horaria al resultado
        st_result = st_result.with_columns([
            pl.lit(station_code).alias("station_code"),
            pl.col("time").dt.replace_time_zone(str(TIMEZONE)).alias("time")
        ])
